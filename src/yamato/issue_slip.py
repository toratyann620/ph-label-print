...
SAMPLE_SHIPMENT = {
    "feed": {
        "entry": [
            {
                "shipment": {
                    "shipment_number":              "SAMPLE-001",
                    "service_type":                 "0",          # 0: 発払い
                    "is_cool":                      "0",
                    ...
                    "package_qty":                  "1",
                    "delivery_time_zone":           "1416",       # 14時〜16時
                    "is_using_shipment_email":      "0",
                    "is_using_delivery_email":      "0",
                    # ─── ご依頼主 ───────────────────────
                    "shipper_telephone_display":    "03-1234-5678",
                    "shipper_name":                 "株式会社サンプル商事",
                    "shipper_zip_code":             "1690072",
                    "shipper_address":              "東京都新宿区大久保1-2-3",
                    "shipper_address4":             "サンプルビル101",
                    "shipper_name_kana":            "ｻﾝﾌﾟﾙｼｮｳｼﾞ",
                    # ─── お届け先 ────────────────────────
...
async def main():
    print("=" * 60)
    print("B2クラウドAPI 送り状発行テスト")
    print(f"検証環境: {BASE_URL}")
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        if r5.status_code == 200 and len(r5.content) > 100:
            pdf_path = f"output_{issue_no}.pdf"
        ...
        print("【テスト結果まとめ】")
        print(f"  発行番号(issue_no): {issue_no}")
        print(f"  伝票番号(tracking_number): {real_tracking or '取得中（検証環境）'}")
        if os.path.exists(f"output_{issue_no}.pdf"):
            print(f"  PDF: output_{issue_no}.pdf")
        print("=" * 60)
